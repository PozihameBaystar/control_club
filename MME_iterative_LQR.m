...
%コントローラーのパラメーター
iLQR.Ts = Ts; %制御周期
iLQR.tf = 2.0; %予測ホライズンの長さ[sec]
iLQR.N = 10; %予測区間の分割数
iLQR.iter = 20; %繰り返し回数の上限値
iLQR.dt = iLQR.tf/iLQR.N; %予測ホライズンの分割幅
iLQR.torelance = 1; %評価関数のズレの許容範囲
iLQR.alpha = 10; %逆温度パラメータ（正確には逆温度の逆数）
iLQR.k = 5; %パーティクルの数
iLQR.m = 10; %リサンプリングの間隔
...
for i = 1:length(tsim)
    ...
    log.u(:,i) = reshape(U_min,[iLQR.len_u*iLQR.N,1]);
    log.x(:,i) = car.x;
    %log.X(:,:,i) = X(:,:,min_id);
    car.x
    ...
    [U_min,U,sig,X] = MME_iLQR_control(iLQR,car);
...
plot(log.x(1,:),log.x(2,:),'LineWidth',1); ylabel('y'); xlabel('x');
xlim([-1,6]); ylim([-1,3]);
viscircles(iLQR.xe1(1:2)',iLQR.radius,'Color','b');
viscircles(iLQR.xe2(1:2)',iLQR.radius,'Color','b');
pbaspect([7 4 1])
%}
...
%MMEiLQRコントローラー
function [U_min,U,sig,X] = MME_iLQR_control(iLQR,car)
    %変数をサンプルの数だけ入れられるように変える
    U = iLQR.U; %前ステップの入力を初期解として使う
    sig = iLQR.sig; %分散を再利用
    X = zeros(iLQR.len_x,iLQR.N+1,iLQR.k);
    J = zeros(1,iLQR.k);
    ...
    for i = 1:iLQR.k
        ...
        J(:,i) = CalcJ(X(:,:,i),U(:,:,i),iLQR); %評価関数の初期値を計算
    end
    ...
    while true
        ...
        %一定間隔でリサンプリング
        if mod(loop,iLQR.m) == 0
            %最小のモードを特定する
            [min_num, min_id] = min(J);
            %新しい入力を入れる変数
            U_ = zeros(iLQR.len_u,iLQR.N,iLQR.k);
            %混合ガウシアンを作ってサンプリング
            for j = 1:iLQR.N
                ave = reshape(U(:,j,:),iLQR.len_u,iLQR.k);
                sig_ = reshape(sig(:,:,j,:),iLQR.len_u,iLQR.len_u,iLQR.k);
                ggm = gmdistribution(ave',sig_,weight);
                rnd = random(ggm,iLQR.k);
                U_(:,j,:) =  rnd';
            end
            %最小のモードはそのままにする
            U_(:,:,min_id) = U(:,:,min_id);
        ...
        %サンプルごとにiLQRの1ステップを並行して行う
        for i = 1:iLQR.k
            %Backward Passの計算
            [K,d,dV1,dV2,sig(:,:,:,i),VH(i)] = ME_Backward(X(:,:,i),U(:,:,i),iLQR,car); 
            %Forward Passの計算
            [X(:,:,i),U(:,:,i),J_new(i)] = Forward(X(:,:,i),U(:,:,i),K,d,dV1,dV2,J(:,i),iLQR,car); 
            %重みは後でオーバー/アンダーフロー防止処理と正規化を施すのでここでは途中まで計算
            weight(i) = J_new(i)+VH(i);
        end

        %重みのオーバー/アンダーフロー防止処理と正規化
        epsilon = 1e-10;
        min_w = min(weight);
        weight = weight - min_w;
        weight = exp(-(1/iLQR.alpha)*weight);
        weight = (1-iLQR.k*epsilon)*weight/sum(weight);
        weight = weight + epsilon;

        loop = loop + 1;

        %{
        if sum(abs(J_new-J)) <= iLQR.torelance %評価関数値が収束して来たら
            J = J_new;
            break
        end
        %}
...
%Backward Pass
function [K,d,dV1,dV2,sig,VH] = ME_Backward(X,U,iLQR,car)
    ...
    sig = zeros(iLQR.len_u,iLQR.len_u,iLQR.N); %入力の分散を入れる変数
    VH = 0; %評価関数のうち、最大エントロピーに関する部分の値を入れる変数
    ...
    for i = iLQR.N:-1:1
        ...
        try chol(Quu);
            ...
            alpa = -min(eig(Quu));
            Quu = Quu + (alpa+1e-0)*eye(iLQR.len_u);
        end

        Qux = Bk'*Sk*Ak;
        
        K_= -pinv(Quu)*Qux; %閉ループゲインの計算
        K(:,:,i) = K_;
        d_ = -pinv(Quu)*Qu'; %開ループフィードバックの計算
        d(:,i) = d_;
        ...
        %分散の記録（念のため疑似逆行列にしておく）
        sig(:,:,i) = iLQR.alpha * 0.5*(pinv(Quu)+pinv(Quu)');

        %最大エントロピーによる部分の更新
        VH = VH + (iLQR.alpha/2)...
            *( log(det(Quu)) - iLQR.len_u*log(2*pi*iLQR.alpha) );
    end
end
